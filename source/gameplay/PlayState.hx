...
class PlayState extends MusicBeatState
{
	...
	{
		#if sys
		var data:BitmapData = BitmapData.fromFile(Sys.getCwd() + "assets/data/songs/" + PlayState.SONG.song.toLowerCase() + '/' + spritePath + ".png");
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			default:
				try {
					dialogue = CoolUtil.coolTextFile(Paths.txt(
					'songs/${SONG.song.toLowerCase()}/${SONG.song.toLowerCase()}Dialogue'
					));
				} catch(e) {
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - MME " + MainMenuState.multiMediaVer : ""), 16);
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			songScoreDef += Math.round(CoolUtil.convertScore(noteDiff));
